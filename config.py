...
# --- VALIDATED VALUES (from walk-forward testing) ---
...
BEARISH_EXIT_PROBABILITY = 0.40  # untested feature (model-triggered early exit) - use with caution

# Trend/RSI/volatility filters: each tested individually and each one
# REDUCED performance vs. the unfiltered model (trend filter alone: 119.1% ->
# 91.4%). Stacked together, they produced ZERO trades across all 5 periods.
# Disabled here to match the validated, tested configuration.
...
# and was consistent across all 5 walk-forward periods. Narrower stops (3-9%)
# cut off good trades early - confirmed by comparing exit-reason counts
# (narrow stops triggered on ~30-50% of trades; the 10% stop triggered on <5%).
STOP_LOSS_PERCENT = 0.10
TAKE_PROFIT_PERCENT = 0.20

# Trailing stop: tested with mixed, inconsistent results (helped in some
# periods, hurt in others) - meaningfully weaker evidence than the fixed
# stop/target combo above. Disabled to match the version actually validated.
